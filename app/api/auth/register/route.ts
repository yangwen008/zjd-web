...
export async function POST(request: Request) {
  try {
    ...
    // 确定角色和状态
    const validRoles = ['user', 'broker', 'village_org', 'project_publisher', 'notary', 'lawyer', 'fengshui'];
    ...
    if (needsReview) {
      ...
      return NextResponse.json({
        success: true,
        pending: true,
        message: `注册成功！您的${applyRole === 'broker' ? '合伙人' : applyRole === 'village_org' ? '村集体' : applyRole === 'notary' ? '公证处' : applyRole === 'lawyer' ? '律师' : applyRole === 'fengshui' ? '风水师' : '大宗用户'}账号正在审核中，审核通过后即可登录。`,
      });
    }
