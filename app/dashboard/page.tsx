...
export default function DashboardPage() {
  ...
  const getCards = () => {
    const cards = [
      { label: '我的发布', value: s.totalAssets, icon: '🏠', color: 'text-blue-600', bg: 'bg-blue-50' },
      { label: '已上架', value: s.approvedAssets, icon: '✅', color: 'text-green-600', bg: 'bg-green-50' },
      { label: '审核中', value: s.pendingAssets, icon: '⏳', color: 'text-orange-600', bg: 'bg-orange-50' },
      { label: '总浏览量', value: s.totalViews, icon: '👀', color: 'text-purple-600', bg: 'bg-purple-50' },
    ];

    if (['broker', 'village_org', 'admin', 'superadmin'].includes(user.role)) {
      cards.push({ label: '线索数', value: s.totalLeads, icon: '📋', color: 'text-cyan-600', bg: 'bg-cyan-50' });
    }
    if (['user', 'broker', 'village_org', 'admin', 'superadmin'].includes(user.role)) {
      cards.push({ label: '收藏数', value: s.totalFavorites, icon: '❤️', color: 'text-rose-600', bg: 'bg-rose-50' });
    }
    if (['project_publisher', 'admin', 'superadmin'].includes(user.role)) {
      cards.push({ label: '大宗项目', value: s.totalBulk, icon: '🏢', color: 'text-yellow-600', bg: 'bg-yellow-50' });
    }
    if (['data_editor', 'admin', 'superadmin'].includes(user.role)) {
      cards.push({ label: '基建数据', value: s.totalInfra, icon: '📡', color: 'text-indigo-600', bg: 'bg-indigo-50' });
    }
  ...
  // 快捷操作
  const quickActions = [
    { label: '发布新资产', href: '/dashboard/assets/new', icon: '➕', roles: ['user', 'broker', 'village_org', 'admin', 'superadmin'] },
    { label: '查看我的资产', href: '/dashboard/assets', icon: '🏠', roles: ['user', 'broker', 'village_org', 'admin', 'superadmin'] },
    { label: '查看线索', href: '/dashboard/leads', icon: '📋', roles: ['broker', 'village_org', 'admin', 'superadmin'] },
    { label: '大宗项目管理', href: '/dashboard/bulk-projects', icon: '🏢', roles: ['project_publisher', 'admin', 'superadmin'] },
    { label: '基建数据管理', href: '/dashboard/infra', icon: '📡', roles: ['data_editor', 'admin', 'superadmin'] },
    { label: '修改个人资料', href: '/dashboard/profile', icon: '👤', roles: ['user', 'broker', 'village_org', 'data_editor', 'project_publisher', 'admin', 'superadmin'] },
    ...
    <div className="max-w-5xl">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {quickActions.map((a) => (
            <Link key={a.href} href={a.href} className="flex items-center space-x-3 p-4 rounded-xl border border-gray-100 hover:border-brand-green/30 hover:bg-brand-green/5 transition-all">
