...
export default async function AssetDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const similar = await getAssets({ province: asset.province || undefined, limit: 3 }).catch(() => []);
  ...
  if ((asset as any).infra_details) {
    try {
      ...
      if (parsed.infra && Array.isArray(parsed.infra) && parsed.infra.length > 0) {
        infraItems = parsed.infra.map((item: any) => ({ icon: item.icon || '', label: item.label || '', status: item.status || '' }));
      }
      if (parsed.env && Array.isArray(parsed.env) && parsed.env.length > 0) {
        envItems = parsed.env.map((item: any) => ({ icon: item.icon || '📌', label: item.label || '', value: item.value || '' }));
      }
    } catch {}
  }
  ...
  const shareConfig = {
    ...
    desc: asset.description ? asset.description.replace(/<[^>]*>/g, '').substring(0, 100) : `${asset.province || ''}·${asset.city || ''} ${asset.area_mu || ''}亩优质资产`,
    imgUrl: shareImage,
  ...
  const jsonLd = {
    ...
    url: `${siteUrl}/asset/${asset.id}`,
    image: shareImage,
    address: { '@type': 'PostalAddress', addressLocality: asset.city || '', addressRegion: asset.province || '', streetAddress: asset.address || '' },
    ...
    offers: { '@type': 'Offer', price: asset.price_year || 0, priceCurrency: 'CNY', availability: 'https://schema.org/InStock' },
  };
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><span className="text-gray-500">面积：</span><span className="font-medium">{asset.area_mu}亩</span></div>
                  <div><span className="text-gray-500">价格：</span><span className="font-medium text-red-600">{asset.price_year}万/年</span></div>
                  <div><span className="text-gray-500">地点：</span><span className="font-medium">{asset.province}{asset.city}{asset.district}</span></div>
                  <div><span className="text-gray-500">类型：</span><span className="font-medium">{asset.asset_type}</span></div>
                </div>
              </div>
                ...
                {similarFiltered.map((item) => (
                  <div key={item.id} className="bg-white rounded-lg shadow overflow-hidden">
                    <div className="h-48 bg-gray-200">
                      {item.images && (
                        <img src={JSON.parse(item.images)[0]?.url || JSON.parse(item.images)[0]?.thumb} alt={item.title} className="w-full h-full object-cover" />
                      )}
                    </div>
                    <div className="p-4">
                      <h3 className="font-semibold mb-2">{item.title}</h3>
                      <div className="text-sm text-gray-600">{item.province}{item.city} {item.area_mu}亩</div>
