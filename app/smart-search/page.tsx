...
function AssetCard({ item }: { item: SearchResult }) {
  ...
  return (
    <Link href={`/asset/${item.id}`} className="block group">
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
        ...
        <div className="p-4">
          <h3 className="font-bold text-gray-900 mb-1 line-clamp-1 group-hover:text-[#1a4731] transition-colors">{item.title}</h3>
...
function BrokerCard({ item }: { item: SearchResult }) {
  const extra = item.extra;
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-bold text-gray-900 truncate group-hover:text-[#1a4731] transition-colors">{item.title}</h3>
                <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200 flex-shrink-0">{ratingLabel}</span>
              </div>
          ...
          {specialties.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {specialties.slice(0, 4).map((s, i) => (
                <span key={i} className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100">{s}</span>
              ))}
            </div>
          )}
...
function BulkCard({ item }: { item: SearchResult }) {
  const extra = item.extra;
  return (
    <Link href={`/bulk-projects/${item.id}`} className="block group">
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
        <div className="relative h-48 overflow-hidden bg-gray-100">
          <img src={item.image || DEFAULT_IMAGE} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
          <div className="absolute top-3 left-3 flex gap-1.5">
...
function ResultCard({ item }: { item: SearchResult }) {
  switch (item.type) {
    case 'broker': return <BrokerCard item={item} />;
    case 'bulk_project': return <BulkCard item={item} />;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {searched && (
          <div className="mb-4 flex items-center justify-between text-sm text-gray-500">
            ...
            {geo && geo.province && (
              <div className="text-xs text-gray-400">📍 {geo.province}内容已置顶 · 按相关度+地理位置综合排序</div>
            )}
          </div>
        )}
