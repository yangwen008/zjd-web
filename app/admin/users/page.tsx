...
interface User {
  ...
  avatar_url: string | null; org_license: string | null; real_name: string | null;
  bio: string | null; daily_quota: number;
    ...
    try {
      ...
      if (data.role && data.role !== editingUser.role) {
        await fetch('/api/admin/users', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'update-role', id: editingUser.id, role: data.role }),
        });
      }
      // 更新资料
      const res = await fetch('/api/admin/users', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'update-profile', id: editingUser.id, ...data }),
      });
      const d = await res.json() as any;
      if (d.success) { show('✅ 用户资料已更新'); setEditingUser(null); fetchUsers(); }
      else { show(`❌ ${d.error}`); throw new Error(d.error); }
    } catch { show('❌ 保存失败'); throw new Error('保存失败'); }
  };
            ...
            : users.length > 0 ? users.map((u) => {
              ...
              return (
                <tr key={u.id} className={`hover:bg-gray-50/50 ${u.status === 'pending' ? 'bg-yellow-50/30' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center space-x-2">
                      <button onClick={() => setEditingUser(u)} className="text-xs text-blue-600 hover:underline">编辑</button>
                      {u.status === 'pending' && (
                        <>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setEditingUser(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <UserEditForm
              user={editingUser as unknown as UserInfo}
              mode="admin"
