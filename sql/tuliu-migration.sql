...
-- 聚土网来源账号
INSERT OR IGNORE INTO source_accounts (name, province, city, user_id, auto_approve, enabled)
VALUES ('聚土网', NULL, NULL, NULL, 1, 1);

-- 创建采集平台用户 (role=project_publisher)
...
INSERT OR IGNORE INTO users (nickname, role, status, daily_quota, created_at, updated_at)
VALUES ('聚土网', 'project_publisher', 'active', 100, datetime('now'), datetime('now'));
...
UPDATE source_accounts SET user_id = (
  SELECT id FROM users WHERE nickname = '聚土网' AND role = 'project_publisher' LIMIT 1
) WHERE name = '聚土网' AND user_id IS NULL;
